...
- `/blog/`, `/learn/`, `/alternatives/` — content hub, built by `content-site/` (Astro), NOT part of the SPA
...
- **Content hub as Astro sub-build** — `content-site/` is a self-contained Astro project (own `package.json`/lockfile) with content collections `blog`, `learn`, `alternatives` (markdown + zod schema: title ≤70, description ≤170, pubDate, updatedDate, faq, ogImage, draft). `ArticleLayout.astro` emits canonical, OG/Twitter, Article + FAQPage JSON-LD, a visible updated date, and a raw-markdown twin per article (`/blog/<slug>.md` via `[slug].md.ts` endpoints). `scripts/merge-content.mts` copies `content-site/dist/{blog,learn,alternatives,_astro}` into `dist/` and appends content routes to `dist/sitemap.xml`. Decision: `.archcore/landing/content-hub-astro-subbuild.adr.md`.
...
3. `vite build` → `dist/`
4. Vite `closeBundle` plugins: `index.html` → `404.html` (SPA fallback), `dist/{route}/index.html` per-route static meta, `dist/sitemap.xml`
5. `build:content`: `npm --prefix content-site run build` → `content-site/dist/`
6. `merge:content`: `npx tsx scripts/merge-content.mts` → copies content sections + `_astro/` assets into `dist/`, appends routes to sitemap

## Examples

Section components live in `src/components/sections/` and use Lingui `<Trans>` and `msg` for all user-facing strings. Per-page documents (e.g. `src/pages/plugin.tsx`, `src/pages/cli.tsx`) call `usePageMeta` once at the top of the page component to set route-specific meta and the matching OG image variant. Content articles are plain markdown files in `content-site/src/content/{blog,learn,alternatives}/<slug>.md` with frontmatter per the collection schema; English only (the content hub is not wired into Lingui).