...
4. `cross-agent-section.tsx` states the cross-agent claim over one flat agent list. It does not compare the plugin with the CLI, and it does not ask the reader to pick.
...
- The home page states the plugin-host set in prose rather than as a card, so a host status change now touches `cross-agent-section.tsx` alongside `plugin-hosts-section.tsx` and `cli-agents-section.tsx`.
