...
A2. **[DONE 2026-07-29, superseded 2026-08-10]** Home title change: meta title "Archcore — repo memory for AI coding agents"; H1/og:title kept the pain phrase; `messaging-alignment.rule.md` amended in the same change. **Reversed by `product/two-discovery-categories`:** the title, H1, og:title and Twitter title are now all "Archcore — Spec-Driven Development & Context Engineering", and memory is retired as positioning.
...
C3. GitHub as SEO surface: org/repo taglines, repo topics, README funnel to archcore.ai. **Taglines and topics are fixed by `product/surface-descriptors`** (CLI, Plugin and GitHub organization tables) as of 2026-08-10 — do not compose new ones here, and do not use the retired "repo memory" phrasing this item originally carried. **README as a search-first surface** (`archcore-ai/cli`, `archcore-ai/plugin` — code lives outside this repo, hence Track C): the first ~120 characters must state the category, the audience and the supported agents; then sections for supported agents, install, comparison with AGENTS.md / CLAUDE.md / Cursor Rules, why git-native, and links to the matching landing and docs pages. README H1s and first blocks were aligned 2026-08-10; the remaining work here is the funnel structure, not the positioning strings.
