...
Create a react.doctor-style product demo: a scripted agent-session animation defined as code (no screen recording) and exported to GIF/WebM/MP4. The scenario lives in **one file** — `scripts/demo-export/demo.html` — driven by a deterministic `seek(t)` function, captured frame-by-frame in headless Chrome, and assembled with ffmpeg. The exported files live in `public/` and are reused in repository READMEs and socials. **No landing page embeds the demo as of 2026-09-12**: the hero showed it as a GIF, then as `<video>` with webm/mp4, and the owner asked for it to be hidden (`landing/home-plain-language-rail.adr.md`). Restoring it is a content decision.
...
- `src/components/sections/hero-section.tsx` — embedded the demo until 2026-09-12, first as `<img src="/demo.gif">`, then as `<video>` with `/demo.webm` and `/demo.mp4` and a `demo-poster.jpg` poster. It embeds nothing now. The assets stay in `public/`.
...
5. **Publish**: copy `out/demo.gif` to `public/demo.gif`, and `out/demo.webm`, `out/demo.mp4`, `out/poster.png` beside it; copy to other repos' READMEs as needed. Prefer WebM/MP4 wherever `<video>` is allowed, it is ~6× smaller at 2× resolution.

## Expected outcome

- A ~18.6s looping agent-session demo in `public/`, as GIF (~2.2 MB, infinite loop) and as webm/mp4. A GIF animates for everyone, including users with OS-level "Reduce Motion" enabled, which freezes rAF-driven live components. Nothing on the site embeds it today.
...
- **A live rAF component silently freezes under "Reduce Motion".** A respectful `prefers-reduced-motion` static-frame fallback reads as "the demo is broken" to users with that OS setting (common on macOS). That is why the hero embedded a GIF instead of the live DOM animation: GIFs play regardless.
