  ...
  const components = [
    {
      ...
      description: _(msg`Architectural decisions with rationale: why this approach, what were alternatives.`),
    },
    {
      icon: Shield,
      title: _(msg`Constraints (must/never)`),
      description: _(msg`Hard rules: what must be done, what is strictly forbidden in the codebase.`),
    },
    {
      icon: Box,
      title: _(msg`Boundaries`),
      description: _(msg`Modules, bounded contexts, interaction interfaces between components.`),
    },
    {
      icon: Code,
      title: _(msg`Examples / Anti-examples`),
      description: _(msg`Samples of correct and incorrect code for specific patterns.`),
    },
    {
      icon: Book,
      title: _(msg`Glossary`),
      description: _(msg`Domain terms and definitions, team's ubiquitous language.`),
    },
    {
      icon: CheckCircle,
    ...
    <SectionContainer id="architecture-record">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
        {components.map((item) => {
          ...
          return (
            <Card key={item.title} className="border-border/50 hover:border-border transition-colors">
