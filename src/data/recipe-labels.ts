...
export const recipeLabels = {
  ...
  tabExamples: label(msg`Examples`),
  ...
  seeExample: label(msg`See an example →`),
  seeBenefits: label(msg`See benefits and limits →`),
  exampleHeading: label(msg`One request, start to finish`),
  exampleRequest: label(msg`Request`),
  /** Tag on a step the agent takes itself, rather than through either tool. */
  exampleAgent: label(msg`Agent`),
