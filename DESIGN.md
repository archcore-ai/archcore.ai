...
> Canonical strings live in the shared context: `product/canonical-narrative` (fixed strings, terminology, avoid-list) and `product/surface-descriptors` (the resolved homepage slots). This section states the brief they create. It does not override them.
...
Never as positioning for Archcore: **memory** (repo memory, agent memory, "a brain for your codebase") and **platform** ("context engineering platform"). The category term *context engineering* is correct; *platform* is what inflates it. Memory stays legal only on comparison pages, attributed to the alternative.
